...
class HelpScoutClient:
    ...
    def __init__(self, config_path, client_id, client_secret, refresh_token, user_agent):
    ...
    def get_access_token(self):
        ...
        if response.status_code != 200:
            ...
            helpscout_response.update({"status": response.status_code})
            raise Exception(
                "Unable to authenticate (HelpScout response: `{}`)".format(helpscout_response)
            )
    ...
    @backoff.on_exception(
        backoff.expo, (Server5xxError, ConnectionError, Server429Error), max_tries=7, factor=3
    )
